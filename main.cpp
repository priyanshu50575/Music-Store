...
public:
    ...
    {
    conn = mysql_init(0);
    conn = mysql_real_connect(conn,"10.1.2.16", "admin", "admin", "musicstore_db", 0, NULL, 0);
    if(conn)
    {
...
{   // Initial Command
    ...
    // Call Connection function to see whether it is connected to our database
...
void AddNewItemInDatabase()
{
    ...
    stringstream streamPrice, streamQuan,streamid;
    string sprice, squan,sid;
    streamid<<id;
    streamid>>sid;
    streamPrice << price;
    streamPrice >> sprice;
    ...
    string insert_query = "insert into musicinfo_tb (m_id, m_category, m_type, m_name, m_artist, m_price, m_quantity) values ('"+sid+"','"+category+"','"+type+"','"+name+"','"+artist+"','"+sprice+"','"+squan+"')";

    const char* q = insert_query.c_str();  //c_str converts string to constant char and this is required because MYSQL only deals with the constant character.
